---
title: "Go SDK Quickstart"
icon: "golang"
---

Get started with the Camb.ai Go SDK in minutes

## Overview

The Camb.ai Go SDK provides a simple interface to integrate high-quality text-to-speech into your applications. This quickstart will have you generating speech in under 5 minutes.

## Installation

```bash
go get github.com/camb-ai/cambai-go-sdk
```

## Authentication

Get your API key from [CAMB.AI Studio](https://studio.camb.ai/) and set it as an environment variable:

```bash
export CAMB_API_KEY=your_api_key_here
```

Or use a `.env` file or load it directly in your code.

## Quick Start

### Streaming Text-to-Speech

Generate and stream speech in real-time:

```go
package main

import (
	"context"
	"io"
	"os"

	"github.com/camb-ai/cambai-go-sdk"
	"github.com/camb-ai/cambai-go-sdk/client"
	"github.com/camb-ai/cambai-go-sdk/option"
)

func main() {
	// Initialize the client
	c := client.NewClient(
		option.WithAPIKey(os.Getenv("CAMB_API_KEY")),
	)

	// Stream TTS audio
	stream, err := c.TextToSpeech.Tts(
		context.Background(),
		&cambai.CreateStreamTtsRequestPayload{
			Text:     "Hello! Welcome to Camb.ai text-to-speech.",
			Language: cambai.TtsLanguageEnUs,
			VoiceID:  147320,
			SpeechModel: cambai.SpeechModelMarsPro.Ptr(),
			OutputConfiguration: &cambai.StreamTtsOutputConfiguration{
				Format: cambai.OutputFormatWav.Ptr(),
			},
		},
	)
	if err != nil {
		panic(err)
	}

	// Save to file
	outFile, _ := os.Create("output.wav")
	defer outFile.Close()
	io.Copy(outFile, stream)
}
```

### Using the Helper Function

Since the SDK returns an `io.Reader`, you can use this helper to save streams easily:

```go
func SaveStreamToFile(stream io.Reader, filename string) error {
	f, err := os.Create(filename)
	if err != nil {
		return err
	}
	defer f.Close()
	_, err = io.Copy(f, stream)
	return err
}

// Usage:
stream, _ := client.TextToSpeech.Tts(...)
SaveStreamToFile(stream, "output.wav")
```

## Choosing a Model

Camb.ai offers three MARS models optimized for different use cases:

<CodeGroup>
```go MARS Flash
stream, _ := c.TextToSpeech.Tts(ctx, &cambai.CreateStreamTtsRequestPayload{
    Text:        "Fast and efficient speech synthesis.",
    Language:    cambai.TtsLanguageEnUs,
    VoiceID:     147320,
    SpeechModel: cambai.SpeechModelMarsFlash.Ptr(),
})
// Best for: Real-time voice agents, low-latency applications
// Sample rate: 22.05kHz
```
```go MARS Pro
stream, _ := c.TextToSpeech.Tts(ctx, &cambai.CreateStreamTtsRequestPayload{
    Text:        "High-fidelity audio production.",
    Language:    cambai.TtsLanguageEnUs,
    VoiceID:     147320,
    SpeechModel: cambai.SpeechModelMarsPro.Ptr(),
})
// Best for: Audio production, high-quality content
// Sample rate: 48kHz
```
```go MARS Instruct
stream, _ := c.TextToSpeech.Tts(ctx, &cambai.CreateStreamTtsRequestPayload{
    Text:             "Great to meet you!",
    Language:         cambai.TtsLanguageEnUs,
    VoiceID:          147320,
    SpeechModel:      cambai.SpeechModelMarsInstruct.Ptr(),
    UserInstructions: cambai.String("Speak in a warm, friendly tone"),
})
// Best for: Fine-grained control over tone and style
// Sample rate: 22.05kHz
```
</CodeGroup>

## Listing Available Voices

Discover available voices for your application:

```go
voices, _ := c.VoiceCloning.ListVoices(
    context.Background(), 
    &cambai.ListVoicesListVoicesGetRequest{},
)

for _, voice := range voices {
    fmt.Printf("ID: %d, Name: %s, Gender: %v\n", voice.ID, voice.VoiceName, voice.Gender)
}
```

## Language Support

Camb.ai supports 140+ languages. Specify the language using the provided constants:
Languages supported by each model mentioned at [MARS Models](https://docs.camb.ai/models).

```go
// English (US)
c.TextToSpeech.Tts(ctx, &cambai.CreateStreamTtsRequestPayload{
    Language: cambai.TtsLanguageEnUs,
    // ...
})

// Spanish
c.TextToSpeech.Tts(ctx, &cambai.CreateStreamTtsRequestPayload{
    Language: cambai.TtsLanguageEsEs,
    // ...
})
```

## Error Handling

Handle common errors gracefully:

```go
stream, err := c.TextToSpeech.Tts(ctx, request)
if err != nil {
    fmt.Printf("Error generating speech: %v\n", err)
    return
}
```

## Using a Custom Hosting Provider

For more details check this guide [Custom Cloud Providers](https://docs.camb.ai/custom-cloud-providers)

### Baseten Mars8-Flash

Set your environment variables and run the [Baseten Provider Example](https://github.com/Camb-ai/cambai-go-sdk/blob/master/examples/baseten-provider/main.go).

Required env vars: `BASETEN_API_KEY`, `BASETEN_URL`, `BASETEN_REFERENCE_AUDIO` (public URL or base64 audio), and optionally `BASETEN_REFERENCE_LANGUAGE` (default: `en-us`).

```go
// reference_audio can be a public URL — Baseten caches it for faster inference
var ttsProvider provider.TtsProvider = &BasetenProvider{
	APIKey:            os.Getenv("BASETEN_API_KEY"),
	URL:               os.Getenv("BASETEN_URL"),
	ReferenceAudio:    os.Getenv("BASETEN_REFERENCE_AUDIO"),
	ReferenceLanguage: "en-us",
}

req := &cambai.CreateStreamTtsRequestPayload{
	Text:     "Hello from the Baseten Mars8-Flash custom hosting provider!",
	Language: cambai.TtsLanguageEnUs,
}

stream, err := ttsProvider.Tts(context.Background(), req)
if err != nil {
	fmt.Printf("Error generating speech: %v\n", err)
	return
}
```

---

## Next Steps

<CardGroup cols={2}>
  <Card title="Voice Agents" icon="/images/pipecat-orange.svg" href="/integrations/pipecat">
    Build real-time voice agents with Pipecat
  </Card>
  <Card title="LiveKit Integration" icon="/images/livekit-orange.svg" href="/integrations/livekit">
    Create voice agents with LiveKit
  </Card>
  <Card title="API Reference" icon="code" href="/api-reference/endpoint/create-tts-stream">
    Explore the full TTS API
  </Card>
  <Card title="Voice Library" icon="speech" href="/api-reference/endpoint/list-voices">
    Browse available voices
  </Card>
</CardGroup>


## Resources

*   [GitHub Repository](https://github.com/Camb-ai/cambai-go-sdk)
*   [SDK Examples](https://github.com/Camb-ai/cambai-go-sdk/tree/master/examples)
*   [API Reference](https://docs.camb.ai/api-reference/endpoint/create-tts-stream)
